...
// Gemini's own Google Search grounding tool is the fallback scraper when Groq is rate-limited —
// same idea as groq/compound, no separate search API key needed here either.
...
/** Gemini's grounding tool does its own web search, then the model synthesises a normal text
 *  reply — no separate response_format needed, but the prompt already demands JSON-only output. */
async function callGeminiGroundedSection(systemPrompt: string, userPrompt: string): Promise<ValidationSection> {
  async function attempt(extra?: string): Promise<ValidationSection> {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 90_000);
    try {
...
interface RunningCheck {
  ...
  markDone(applicationId: string, result: ValidationSection, model: string): Promise<void>;
...
async function runSection(applicationId: string, section: SectionKey): Promise<{ usedModel: string }> {
  if (!process.env.GROQ_API_KEY && !process.env.GEMINI_API_KEY) {
    throw new Error('Neither GROQ_API_KEY nor GEMINI_API_KEY is configured — organisation validation needs one of them.');
  }
  ...
  try {
    ...
    const userPrompt = buildPrompt(app);

    let result: ValidationSection;
    let usedModel: string;
    if (process.env.GROQ_API_KEY) {
      try {
        result = await callGroqCompoundSection(systemPrompt, userPrompt);
        usedModel = GROQ_COMPOUND_MODEL;
      } catch (err) {
        if (!process.env.GEMINI_API_KEY) throw err;
        result = await callGeminiGroundedSection(systemPrompt, userPrompt);
        usedModel = `${GEMINI_MODEL} (groq fallback)`;
      }
    } else {
      result = await callGeminiGroundedSection(systemPrompt, userPrompt);
    ...
    await check.markDone(applicationId, result, usedModel);
    return { usedModel };
  } catch (err) {
